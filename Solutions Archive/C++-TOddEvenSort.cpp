...
template <typename T>
void oddEvenSort(std::vector<T>& v) {
	int sorted = -1;
	while (sorted != 0) {
		sorted = 0;
		//odd first
		for (typename std::vector<T>::iterator it = v.begin() + 1; it < v.end() - 1; it += 2) {
			if (*it > *(it + 1)) {
		...
		//even second
		for (typename std::vector<T>::iterator it = v.begin(); it < v.end() - 1; it += 2) {
			if (*it > *(it + 1)) {
				std::iter_swap(it, it + 1);
				sorted += 1;
			}
